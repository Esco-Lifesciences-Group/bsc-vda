...
int BlowerRbmDsi::getInterlock() const
{
...
void BlowerRbmDsi::readActualSpeedRPM()
{
    ...
    //READ_ACTUAL_RPM
    int actualRPM = 0/*, actualTorque = 0*/;
    //    response = pModule->getTorque(&actualTorque);

    //    qDebug() << "response" << response << "actualTorque"  << actualTorque;
    ...
    if(response == 0){
        ...
        actualRPM       = m_rpmSamplesSum / (m_rpmSamples.length() != 0 ? m_rpmSamples.length() : 1);
...
{
    if(dummyState >= m_dutyCycleLimit)
        dummyState = m_dutyCycleLimit;
...
void BlowerRbmDsi::setDutyCycleLimit(int value)
{
...
void BlowerRbmDsi::setDutyCycle(int newVal)
{
    ...
    if(newVal >= m_dutyCycleLimit)
        newVal = m_dutyCycleLimit;
