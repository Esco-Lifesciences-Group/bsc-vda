...
#define URL_AUTO_UPDATE "172.22.10.23:8080/Factory-Production-Line-1/Project_SBC_Software/Auto_Update/"
//#define URL_RP_DATABASE "172.22.10.23:8080/Factory-Production-Line-1/Project_SBC_Software/RP-Database/"
...
CheckSWUpdate::CheckSWUpdate(QObject *parent) : ClassManager(parent)
{
    ...
    QString a, b;
    a = targetLocation + "/sw-latest-version";
    b = ":/Generic/sw-latest-version";
    QFile swLatestVersion(a);

    if(!swLatestVersion.exists())
        QFile::copy(b, a);
}//
...
void CheckSWUpdate::routineTask(int parameter)
{
    ...
    if(updateFile.exists()){
        ...
        if(_isSwLatestVersionFileValid(updateFile, swuFileName)){
            ...
            /// Output the file with different name
    ...
    if(!exitCode){
        ...
        if(m_newSoftwareVersionRelease){
            ...
            //            /// Double check the md5 checksum (Not Requiered Cause SomeTimes the MD5Sum of the file is changed after powered Off)
            //            /// Check the actual md5 checksum of the existing swu file in local repository (Not Requiered)
            //            /// is it matched with md5 checksum on sw-latest-version? (Not Requiered)
            ...
            qDebug() << cmd;
            qprocess.start(cmd);
            qprocess.waitForFinished(5*60000); /// wait about maximum 5 minutes
...
QJsonObject CheckSWUpdate::getSwUpdateHistory(bool autoCheckEn)
{
    QString targetDir = QStandardPaths::writableLocation(QStandardPaths::AppLocalDataLocation);
    targetDir += "/swupdate";
...
void CheckSWUpdate::refreshRpExternalDatabase()
{
#ifdef __arm__
    QString urlRpDatabase = m_rpExtDatabaseUrl;
    if(urlRpDatabase.isEmpty()) return;

    QString urlAuthentication = QString("http://%1:%2@").arg(USERNAME, PASSWORD);
    QString database = urlRpDatabase.split("/")[urlRpDatabase.split("/").length()-1];
    QString databaseTemp = database + "_temp";

    QString targetDir = "/home/root/resources";

    QString url =  urlAuthentication + urlRpDatabase /*+ database*/;

    QString cmd = QString("wget -O %1 %2").arg((targetDir + "/" + databaseTemp), url);
    ...
    /// Check the file
    if(!exitCode){
        QFile oldFile(targetDir + "/" + database);
        oldFile.remove();

        QFile newFile(targetDir + "/" + databaseTemp);
        QString newName = newFile.fileName();
        newName.replace("_temp", "");
        newFile.rename(newName);
    }else{
        qWarning() << exitCode << output;
    }
#endif
    getRpExtDatabase();
}

void CheckSWUpdate::refreshRpExternalDefault()
{
#ifdef __arm__
    QString urlRpDefault = m_rpExtDefaultUrl;
    if(urlRpDefault.isEmpty()) return;

    QString urlAuthentication = QString("http://%1:%2@").arg(USERNAME, PASSWORD);
    QString database = urlRpDefault.split("/")[urlRpDefault.split("/").length()-1];
    QString databaseTemp = database + "_temp";

    QString targetDir = "/home/root/resources";

    QString url =  urlAuthentication + urlRpDefault /*+ database*/;
...
void CheckSWUpdate::getRpExtDatabase()
{
    QJsonObject jsonObj;
    /// path: "/home/root/resources/rp-database"
    QString filePath = "/home/root/resources/rp-database";
#ifndef __arm__
    filePath = QDir::currentPath() + "/rp-database";
#endif
    ...
    emit rpExtDatabaseObtained(jsonObj);
}

void CheckSWUpdate::getRpExtDefault()
{
    QJsonObject jsonObj;
    /// path: "/home/root/resources/rp-database"
    QString filePath = "/home/root/resources/rp-default";
#ifndef __arm__
    filePath = QDir::currentPath() + "/rp-default";
#endif
    ...
    emit rpExtDefaultObtained(jsonObj);
}//
...
bool CheckSWUpdate::_hasNewSwuFileMd5SumChanged()
{
    ...
    if(jsonDataObject[m_currentSoftwareName].isUndefined()){
        ...
        if(latestVerMd5SumValid){
            ...
            qDebug() << "latestVerMd5SumValid is invalid!";
            return false;
        }
    }//
    ...
    if(newVersion <= currentVersion) {
        if(newVersion == currentVersion){
            ...
            if(!_hasNewSwuFileMd5SumChanged()){
                ...
                bool swuFileToBeDownloadedExist = QFile().exists(targetDir + QString("/%1").arg(swuFileNametoBeDownloaded));
                if(!swuFileToBeDownloadedExist){
                    qDebug() << QString("Return false: there is no %1 file in the local repository yet").arg(swuFileNametoBeDownloaded);
                    qDebug() << "Software version is the same, no need to update";
                    return false;
                }
                else{
                    qDebug() << QString("Return true: there is a %1 file in the local repository").arg(swuFileNametoBeDownloaded);
                }
            }else{
...
bool CheckSWUpdate::_isSwLatestVersionFileValid(QFile &file, QString &swuFileName)
{
    ...
    if(jsonDataObject[m_currentSoftwareName].isUndefined()){
        ...
        if(!versionValid){
            ...
            m_newSoftwareVersion += QString(".%1.%2").arg(QString::number(m_newPatchVersion), QString::number(m_newBuildVersion));
            m_newSoftwareSwuUrl = jsonDataObject["latestVerUrl"].toString();
            m_newSoftwareSwuMd5Sum = jsonDataObject["latestVerMd5Sum"].toString();
            m_newSoftwareHistoryUrl = jsonDataObject["historyVerUrl"].toString();
            m_newSoftwareVersionRelease = jsonDataObject["release"].toBool();

            m_rpExtDatabaseUrl = jsonDataObject["rpDatabaseUrl"].toString();
            m_rpExtDefaultUrl = jsonDataObject["rpDefaultUrl"].toString();
...
QString CheckSWUpdate::_fileChecksum(const QString &fileName, QCryptographicHash::Algorithm hashAlgorithm)
{
    ...
    qDebug() << app << QStringList() << fileName;
    qDebug() << output;
    return output;
}//
