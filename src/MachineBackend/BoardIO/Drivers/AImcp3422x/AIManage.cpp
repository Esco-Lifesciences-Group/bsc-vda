        ...
        if(response == 0){
            if(m_pAIModule->isConversionUpdated()){
                if(channelsDoAverage[index] == 1){
                    ...
                    int sampleLength = channelsSamples[index].length();
                    if(sampleLength > 0)
                        channelsADC[index]       = qRound(static_cast<double>(channelTotalADC[index]) / (static_cast<double>(sampleLength)));
...
{
    /// 4095 -> 10.118 Volt
    int mVolt = map(adc, 0, MAXIMUM_ADC_RESOLUTION, 0, 10118);
    if(mVolt >= 10118) mVolt = 10118;
    else if(mVolt < 0) mVolt = 0;
    return mVolt;
