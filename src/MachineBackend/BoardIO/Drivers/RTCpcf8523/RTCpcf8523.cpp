...
enum PCF8523_REG{
    ...
    PCF8523_REG_SECOND, //03h
    PCF8523_REG_MINUTE,
    PCF8523_REG_HOURS,
    PCF8523_REG_DAYS,
    PCF8523_REG_WEEKDAYS,
    PCF8523_REG_MONTHS,
    PCF8523_REG_YEARS,//09h
...
int RTCpcf8523::init()
{
    ...
    if(invalid_flag & 0x80){
        ...
        /* PM[2:0]-> Control_3bit = 7-0 = PM[2:0],-,BSF,BLF,BSIE,BLIE, in register Control_3 (02h):
         ...
         * 010,011[1] : battery switch-over function is disabled - only one power supply (VDD); battery low detection function is enabled
         * 100 : battery switch-over function is enabled in standard mode; battery low detection function is disabled - selected
         * 101 : battery switch-over function is enabled in direct switching mode; battery low detection function is disabled
         * 110 : not allowed
         * 111 : battery switch-over function is disabled - only one power supply (VDD); battery low detection function is disabled*/

        cmd.push_back(0x00); // power swith mode, battery switch over
        //cmd.push_back(0x80); // PM[2:0]-> 100
        cmd.push_back(0x00); // second
        cmd.push_back(0x00); // minute
        cmd.push_back(0x00); // hour
        cmd.push_back(0x01); // day
        cmd.push_back(0x00); // wday
        cmd.push_back(0x01); // month
        cmd.push_back(0x00); // year
        //call i2c object and pass command frame
        if(pI2C->writeData(cmd) != I2CPort::I2C_COMM_RESPONSE_OK)
        {
            return I2CPort::I2C_COMM_RESPONSE_ERROR;
        }
        secOscilationIsValid = false;
    }
    else {
        secOscilationIsValid = true;
    }
    return I2CPort::I2C_COMM_RESPONSE_OK;
}
...
int RTCpcf8523::setDateTime(int weekday, int day, int month, int year,
                            int hour, int minute, int second, bool toBuffer)
{
    //formated value
    unsigned char bcd_sec = dec_to_bcd(second);
    unsigned char bcd_min = dec_to_bcd(minute);
...
int RTCpcf8523::getDateTime(int &wday, int &day, int &month, int &year,
...
int RTCpcf8523::getBatteryStatus(int &status)
{
    //generate comand frame
    vector<unsigned char> cmd;
    pI2C->generateFrame(I2CPort::I2C_CMD_OPERATION_READ,
    ...
    //update register data
    m_registerDataBuffer[PCF8523_REG_CONTROL_3]     = receive[0];
    status = (m_registerDataBuffer[PCF8523_REG_CONTROL_3] & 0x04) ? PCF8523_BATTERY::STATUS_LOW
                                                                  : PCF8523_BATTERY::STATUS_OK;
...
int RTCpcf8523::polling()
{
    ...
    ////GET DATE TIME
    {
        ...
        response = getDateTime(wday, day, month, year, hour, minute, second);
    }
    //    ////GET TIME
    //    {
...
bool RTCpcf8523::isSecOscilationIsValid() const
{
    return secOscilationIsValid;
}
