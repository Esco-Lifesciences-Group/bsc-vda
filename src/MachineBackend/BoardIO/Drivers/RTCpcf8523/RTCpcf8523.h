...
public:
    ...
    int init() override;
    int testComm() override;
    int polling() override;
    ...
    /// Recommended to access date time in one go - PCF8523 product datasheet page 24/78
    int setDateTime(int weekday, int day, int month, int year,
                    int hour, int minute, int second = 0, bool toBuffer = false);
    ...
    /// Recommended to access date time in one go - PCF8523 product datasheet page 24/78
    int getDateTime(int& wday, int& day, int& month, int& year,
                    int& hour, int& minute, int& second);

    int getTimerACount(int &count);
    /// 1 low, 0 OK, control Register 3 bit 2
    int getBatteryStatus(int &status);
    ...
    bool isSecOscilationIsValid() const;

private:
    int bcd_to_dec(unsigned char val);
    unsigned char dec_to_bcd(int val);

    bool secOscilationIsValid = false;
};
