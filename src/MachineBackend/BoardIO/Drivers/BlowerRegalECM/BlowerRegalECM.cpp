...
int BlowerRegalECM::getTorque(int *torque)
{
    if (!isPortValid()) return -1;
    ...
    cmd.append(ECM_CMD_REQ_TORQUE_RTQ);
    ...
    if(serialComm->waitForBytesWritten(ECM_TRANS_RESPONSE_TIME)){
        if(serialComm->waitForReadyRead(ECM_TRANS_RESPONSE_TIME)){
            ...
            if(response.length() == 10){
                if(isChecksumValid(response)){
                    int _torque = (response[6] << 8) | response[5];
                    int _torqueNm = _torque / 1;
                    *torque = _torqueNm;

                    printf("BlowerRegalECM::getTorque _torqueNm: %d\n", _torqueNm);
                    fflush(stdout);
                }else return -1;
            }
            else if (response.length() == 9){
