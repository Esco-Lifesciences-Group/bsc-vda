...
ViewApp {
    ...
    content.sourceComponent: ContentItemApp {
                                ...
                                ColumnLayout{
                                    ...
                                    Item {
                                        ...
                                        ComboBoxApp {
                                            ...
                                            //                                        let repeat = props.lightAutoSetDayRepeat
                                            //                                        let day = props.lightAutoSetWeeklyDay
                        ...
                        ButtonBarApp {
                            ...
                            onClicked: {
                                if(props.parameterOnHasChanged){
                                    ...
                                    MachineAPI.setLightAutoTime(props.onTimeSet)
                                    MachineAPI.setLightAutoDayRepeat(props.onRepeatSet)
                                    MachineAPI.setLightAutoWeeklyDay(props.onRepeatDaySet)
                                    MachineAPI.setLightAutoEnabled(props.onEnableSet)

                                    const setStrf = "%1 -  %2".arg(props.onTimeSetStrf).arg(props.onRepeatSetStrf)
                                    const message = props.onEnableSet ? qsTr("User: Light On Scheduler enabled (%1)").arg(setStrf)
                                                                      : qsTr("User: Light On Scheduler disabled")
                                    MachineAPI.insertEventLog(message)
                                }
                                if(props.parameterOffHasChanged){
                                    //console.debug("Off setting")
                                    MachineAPI.setLightAutoTimeOff(props.offTimeSet)
                                    MachineAPI.setLightAutoDayRepeatOff(props.offRepeatSet)
                                    MachineAPI.setLightAutoWeeklyDayOff(props.offRepeatDaySet)
                                    MachineAPI.setLightAutoEnabledOff(props.offEnableSet)

                                    const setStrf = "%1 -  %2".arg(props.offTimeSetStrf).arg(props.offRepeatSetStrf)
                                    const message = props.offEnableSet ? qsTr("User: Light Off Scheduler enabled (%1)").arg(setStrf)
                                                                       : qsTr("User: Light Off Scheduler disabled")
                                    MachineAPI.insertEventLog(message)
                                }
                                ...
                                showBusyPage(qsTr("Setting up..."), function(cycle){
                                    let valueSet = true
                                    if(props.parameterOnHasChanged){
                                        valueSet &= (MachineData.lightAutoSetTime        === props.onTimeSet
                                                     && MachineData.lightAutoSetDayRepeat=== props.onRepeatSet
                                                     && MachineData.lightAutoSetWeeklyDay=== props.onRepeatDaySet
                                                     && MachineData.lightAutoSetEnabled  === props.onEnableSet)

                                    }
                                    if(props.parameterOffHasChanged){
                                        valueSet &= (MachineData.lightAutoSetTimeOff        === props.offTimeSet
                                                     && MachineData.lightAutoSetDayRepeatOff=== props.offRepeatSet
                                                     && MachineData.lightAutoSetWeeklyDayOff=== props.offRepeatDaySet
                                                     && MachineData.lightAutoSetEnabledOff  === props.offEnableSet)
        ...
        QtObject {
            ...
            onOnEnableSetChanged:   {if(onEnableSet != MachineData.lightAutoSetEnabled) parameterOnHasChanged |= 0x0001;else parameterOnHasChanged &= ~0x0001}
            onOnTimeSetChanged:     {if(onTimeSet != MachineData.lightAutoSetDayRepeat) parameterOnHasChanged |= 0x0002;else parameterOnHasChanged &= ~0x0002}
            onOnRepeatSetChanged:   {if(onRepeatSet != MachineData.lightAutoSetDayRepeat) parameterOnHasChanged |= 0x0004;else parameterOnHasChanged &= ~0x0004}
            onOnRepeatDaySetChanged:{if(onRepeatDaySet != MachineData.lightAutoSetWeeklyDay) parameterOnHasChanged |= 0x0008;else parameterOnHasChanged &= ~0x0008}
            ...
            onOffEnableSetChanged:   {if(offEnableSet != MachineData.lightAutoSetEnabledOff) parameterOffHasChanged |= 0x0001;else parameterOffHasChanged &= ~0x0001}
            onOffTimeSetChanged:     {if(offTimeSet != MachineData.lightAutoSetTimeOff) parameterOffHasChanged |= 0x0002;else parameterOffHasChanged &= ~0x0002}
            onOffRepeatSetChanged:   {if(offRepeatSet != MachineData.lightAutoSetDayRepeatOff) parameterOffHasChanged |= 0x0004;else parameterOffHasChanged &= ~0x0004}
            onOffRepeatDaySetChanged:{if(offRepeatDaySet != MachineData.lightAutoSetWeeklyDayOff) parameterOffHasChanged |= 0x0008;else parameterOffHasChanged &= ~0x0008}

            property int parameterOnHasChanged: 0
            property int parameterOffHasChanged: 0

            property int timePeriodMode: 24 //12h
            property string lightAutoTimeText: ""
        ...
        Component.onCompleted: {
            ...
            const onMinutesSchedule = MachineData.lightAutoSetTime
            const offMinutesSchedule = MachineData.lightAutoSetTimeOff
            ...
            let onRepeat = MachineData.lightAutoSetDayRepeat
            let offRepeat = MachineData.lightAutoSetDayRepeatOff

            props.onRepeatSet = onRepeat
            props.offRepeatSet = offRepeat

            let onDay = MachineData.lightAutoSetWeeklyDay
            let offDay = MachineData.lightAutoSetWeeklyDayOff
            ...
            props.onEnableSet = MachineData.lightAutoSetEnabled
            props.offEnableSet = MachineData.lightAutoSetEnabledOff

            onEnableSwitch.checked = MachineData.lightAutoSetEnabled
            onEnableSwitch.initialized = true
            offEnableSwitch.checked = MachineData.lightAutoSetEnabledOff
