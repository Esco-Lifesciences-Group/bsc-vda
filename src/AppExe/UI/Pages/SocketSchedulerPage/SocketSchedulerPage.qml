...
ViewApp {
    id: viewApp
    title: "Electrical Outlet Scheduler"
    ...
    content.sourceComponent: ContentItemApp {
        ...
        ColumnLayout {
            ...
            Item {
                ...
                HeaderApp {
                    anchors.fill: parent
                    title: qsTr("Electrical Outlet Scheduler")
                }
            }
                                ...
                                ColumnLayout{
                                    ...
                                    Item {
                                        ...
                                        ComboBoxApp {
                                            ...
                                            //                                        let repeat = props.socketAutoSetDayRepeat
                                            //                                        let day = props.socketAutoSetWeeklyDay
                        ...
                        ButtonBarApp {
                            ...
                            onClicked: {
                                if(props.parameterOnHasChanged){
                                    ...
                                    MachineAPI.setSocketAutoTime(props.onTimeSet)
                                    MachineAPI.setSocketAutoDayRepeat(props.onRepeatSet)
                                    MachineAPI.setSocketAutoWeeklyDay(props.onRepeatDaySet)
                                    MachineAPI.setSocketAutoEnabled(props.onEnableSet)

                                    const setStrf = "%1 -  %2".arg(props.onTimeSetStrf).arg(props.onRepeatSetStrf)
                                    const message = props.onEnableSet ? qsTr("User: Outlet On Scheduler enabled (%1)").arg(setStrf)
                                                                      : qsTr("User: Outlet On Scheduler disabled")
                                    MachineAPI.insertEventLog(message)
                                }
                                if(props.parameterOffHasChanged){
                                    //console.debug("Off setting")
                                    MachineAPI.setSocketAutoTimeOff(props.offTimeSet)
                                    MachineAPI.setSocketAutoDayRepeatOff(props.offRepeatSet)
                                    MachineAPI.setSocketAutoWeeklyDayOff(props.offRepeatDaySet)
                                    MachineAPI.setSocketAutoEnabledOff(props.offEnableSet)

                                    const setStrf = "%1 -  %2".arg(props.offTimeSetStrf).arg(props.offRepeatSetStrf)
                                    const message = props.offEnableSet ? qsTr("User: Outlet Off Scheduler enabled (%1)").arg(setStrf)
                                                                       : qsTr("User: Outlet Off Scheduler disabled")
                                    MachineAPI.insertEventLog(message)
                                }
                                ...
                                showBusyPage(qsTr("Setting up..."), function(cycle){
                                    let valueSet = true
                                    if(props.parameterOnHasChanged){
                                        valueSet &= (MachineData.socketAutoSetTime        === props.onTimeSet
                                                     && MachineData.socketAutoSetDayRepeat=== props.onRepeatSet
                                                     && MachineData.socketAutoSetWeeklyDay=== props.onRepeatDaySet
                                                     && MachineData.socketAutoSetEnabled  === props.onEnableSet)

                                    }
                                    if(props.parameterOffHasChanged){
                                        valueSet &= (MachineData.socketAutoSetTimeOff        === props.offTimeSet
                                                     && MachineData.socketAutoSetDayRepeatOff=== props.offRepeatSet
                                                     && MachineData.socketAutoSetWeeklyDayOff=== props.offRepeatDaySet
                                                     && MachineData.socketAutoSetEnabledOff  === props.offEnableSet)
        ...
        QtObject {
            ...
            onOnEnableSetChanged:   {if(onEnableSet != MachineData.socketAutoSetEnabled) parameterOnHasChanged |= 0x0001;else parameterOnHasChanged &= ~0x0001}
            onOnTimeSetChanged:     {if(onTimeSet != MachineData.socketAutoSetDayRepeat) parameterOnHasChanged |= 0x0002;else parameterOnHasChanged &= ~0x0002}
            onOnRepeatSetChanged:   {if(onRepeatSet != MachineData.socketAutoSetDayRepeat) parameterOnHasChanged |= 0x0004;else parameterOnHasChanged &= ~0x0004}
            onOnRepeatDaySetChanged:{if(onRepeatDaySet != MachineData.socketAutoSetWeeklyDay) parameterOnHasChanged |= 0x0008;else parameterOnHasChanged &= ~0x0008}
            ...
            onOffEnableSetChanged:   {if(offEnableSet != MachineData.socketAutoSetEnabledOff) parameterOffHasChanged |= 0x0001;else parameterOffHasChanged &= ~0x0001}
            onOffTimeSetChanged:     {if(offTimeSet != MachineData.socketAutoSetTimeOff) parameterOffHasChanged |= 0x0002;else parameterOffHasChanged &= ~0x0002}
            onOffRepeatSetChanged:   {if(offRepeatSet != MachineData.socketAutoSetDayRepeatOff) parameterOffHasChanged |= 0x0004;else parameterOffHasChanged &= ~0x0004}
            onOffRepeatDaySetChanged:{if(offRepeatDaySet != MachineData.socketAutoSetWeeklyDayOff) parameterOffHasChanged |= 0x0008;else parameterOffHasChanged &= ~0x0008}

            property int parameterOnHasChanged: 0
            property int parameterOffHasChanged: 0

            property int timePeriodMode: 24 //12h
            property string socketAutoTimeText: ""
        ...
        Component.onCompleted: {
            ...
            const onMinutesSchedule = MachineData.socketAutoSetTime
            const offMinutesSchedule = MachineData.socketAutoSetTimeOff
            ...
            let onRepeat = MachineData.socketAutoSetDayRepeat
            let offRepeat = MachineData.socketAutoSetDayRepeatOff

            props.onRepeatSet = onRepeat
            props.offRepeatSet = offRepeat

            let onDay = MachineData.socketAutoSetWeeklyDay
            let offDay = MachineData.socketAutoSetWeeklyDayOff
            ...
            props.onEnableSet = MachineData.socketAutoSetEnabled
            props.offEnableSet = MachineData.socketAutoSetEnabledOff

            onEnableSwitch.checked = MachineData.socketAutoSetEnabled
            onEnableSwitch.initialized = true
            offEnableSwitch.checked = MachineData.socketAutoSetEnabledOff
