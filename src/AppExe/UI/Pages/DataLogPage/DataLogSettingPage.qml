    ...
    content.sourceComponent: ContentItemApp {
            ...
            Item {
                            ...
                            Item {
                                ...
                                SwitchApp {
                                    ...
                                    onCheckedChanged: {
                                        ...
                                        MachineAPI.setDataLogEnable(checked)
                                        MachineAPI.setDataLogRunning(checked)

                                        MachineAPI.insertEventLog(qsTr("User: Set data log enable to %1").arg(checked))
                                    }//
                                }//
                            }//
                        }//
                        ...
                        ColumnLayout {
                            ...
                            Item {
                                ...
                                ComboBoxApp {
                                    ...
                                    onActivated: {
                                        MachineAPI.setDataLogPeriod(model[index].value)

                                        MachineAPI.insertEventLog(qsTr("User: Set data log period to %1").arg(model[index].text))
                                    }//
                                }//
                            }//
                                        ...
                                        MouseArea {
                                            ...
                                            onClicked: {
                                                        ...
                                                        && UserSessionService.roleLevel != UserSessionService.roleLevelService)
                                                {
                                                    ...
                                                    showDialogAsk(qsTr("Data Log Options"), message, dialogAlert,
                                                                  function onAccepted(){
                                                                      KeyboardOnScreenCaller.openNumpad(exportNumberPageTextEdit, qsTr("Page to Export (ex. 1 or 1-10, max. 10 pages per export)"))
                                                                  },
                                                ...
                                                if(UserSessionService.roleLevel >= UserSessionService.roleLevelAdmin
                                                    ...
                                                    showDialogAsk(qsTr(title),
                                                                  ...
                                                                  function onAccepted(){
                                                                      ...
                                                                      MachineAPI.insertEventLog(qsTr("User: Delete data logs starting from %1").arg(props.deleteWhereOlderThanDaysStrf))
                                                                  });
                                                }else
                                                    showDialogMessage(qsTr("Access Denied"),
                                                                      qsTr("You do not have permission to perform this action!"),
                ...
                TextApp{
                    ...
                    MouseArea{
                        ...
                        anchors.fill: parent
                        onClicked: {
                            const intent = IntentApp.create("qrc:/UI/Pages/BackupRestorePage/BackupRestorePage.qml", {"db": "datalog.db"})
                            startView(intent)
                        }//
                    }//
                }//
        ...
        Loader {
            ...
            sourceComponent: DataLogQmlApp {
                ...
                Component.onCompleted: {
                    const uniqConnectionName = "dataLogUIforDelete"
        ...
        Loader {
            ...
            sourceComponent: DataLogQmlApp {
                ...
                onLogHasExported: {
                    ...
                    const strEvent = qsTr("User: Generate data logs (export file) %1.").arg(qsTr("(Page: %1-%2)").arg(props.startPageNumberExport).arg(props.endPageNumberExport))
                    MachineAPI.insertEventLog(strEvent)
                }

                Component.onCompleted: {
                    showBusyPage(qsTr("Please wait..."))
                    const uniqConnectionName = "dataLogUIforExport"
                    init(uniqConnectionName);
                    setSeasPressureInstalled(MachineData.seasInstalled);
                }//
            }//
        }//
        ...
        executeOnPageVisible: QtObject {
            ...
            Component.onCompleted: {
                ...
                const period = MachineData.dataLogPeriod
                let periodIndex = 0
                switch (period){
                case 5:
                    periodIndex = 1
