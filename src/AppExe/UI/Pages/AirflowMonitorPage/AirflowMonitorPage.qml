                        ...
                        ComboBoxApp {
                            ...
                            onActivated: {
                                ...
                                if(newValue !== props.airflowMonitorEnable){
                                    if(newValue === 0)
                                        message = qsTr("By disabling the Airflow Monitor, the airflow fail alarm will not be monitored.")
                                    else
                                        message = qsTr("By enabling the Airflow Monitor, the airflow fail alarm will be monitored.")
                                    ...
                                    showDialogAsk(qsTr("Airflow Monitor"), message, (newValue ? dialogInfo : dialogAlert),
                                                  function onAccepted(){
                                                      ...
                                                      const stringEvent = newValue ? qsTr("User: Airflow monitor is enabled")
                                                                                 : qsTr("User: Airflow monitor is disabled")
                                                      MachineAPI.insertEventLog(stringEvent)

                                                      viewApp.showBusyPage(qsTr("Setting up..."),
                                                                           function(cycle){
                                                                               if(cycle >= MachineAPI.BUSY_CYCLE_1
                                                                                       || Number(MachineData.airflowMonitorEnable) == Number(newValue)){
