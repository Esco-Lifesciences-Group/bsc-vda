                ...
                TextInput{
                    ...
                    onAccepted: {
                        if((cabinetNameBufferTextInput.length !== 0) && (props.cabinetName !== cabinetNameBufferTextInput.text)){
                            ...
                            MachineAPI.insertEventLog(qsTr("User: Set cabinet name to %1").arg(cabinetNameBufferTextInput.text))

                            viewApp.showBusyPage(qsTr("Setting up..."),
                                                 function onTriggered(cycle){
                                                     if(cycle >= MachineAPI.BUSY_CYCLE_1
                                                             || MachineData.cabinetDisplayName === cabinetNameBufferTextInput.text){
                                                         viewApp.closeDialog()
                                                     }
                                                 })
                        }
