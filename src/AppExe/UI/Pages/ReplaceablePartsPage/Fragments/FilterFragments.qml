...
Item {
    ...
    ColumnLayout{
        ...
        Item{
            ...
            RowLayout{
                ...
                Item{
                    Layout.fillHeight: true
                    Layout.minimumWidth: props.modelColumnTitle.length == 8 ? 100 : 150
                    TextBoxApp{
                        anchors.fill: parent
                ...
                Item{
                    ...
                    visible: props.modelColumnTitle.length == 8
                    TextBoxApp{
                        anchors.fill: parent
                        text: props.modelColumnTitle[6]
                        boxOpacity: 1
                    }
                }
                Item{
                        ...
                        Repeater{
                            ...
                            CusComPage.RowItemApp {
                                ...
                                onLoaded: {
                                    modelColumn = Qt.binding(function(){
                                        let model = [
                                                ...
                                                props.getRpList(indexOnList+(props.maxRow*5)),
                                                props.getRpList(indexOnList+(props.maxRow*6)),
                                                Number(props.getRpList(indexOnList+(props.maxRow*7))) ? true : false
                                            ]//
                                        return model
                                    })//
                                }//
                                ...
                                onSoftwareConstAccepted: {
                                    /// Set Filter Size
                                    MachineAPI.setReplaceablePartsSettings(indexOnList+(props.maxRow*5), value)
                                }
                                onFilterAirflowAccepted: {
                                    MachineAPI.setReplaceablePartsSettings(indexOnList+(props.maxRow*6), value)
                                }
                                onCheckedChanged: {
                                    const strVal = value ? "1" : "0"
                                    if((props.getRpList(indexOnList+(props.maxRow*7)) === strVal )
                                            || (strVal === "0" && props.getRpList(indexOnList+(props.maxRow*7)) === "")) return
                                    MachineAPI.setReplaceablePartsSettings(indexOnList+(props.maxRow*7), (value ? "1" : "0"))
                                }
                                onIncompleteData: {
                                ...
                                onItemShiftUpClicked: {
                                    if(indexOnList > props.startIndex){
                                        ...
                                        const backUpSize = props.getRpList(prevIndex+(props.maxRow*4))
                                        const backUpAirflow = props.getRpList(prevIndex+(props.maxRow*5))
                                        const backUpPD = props.getRpList(prevIndex+(props.maxRow*6))
                                        const backUpCheck =  props.getRpList(prevIndex+(props.maxRow*7))
                                        ...
                                        MachineAPI.setReplaceablePartsSettings(Number(prevIndex+(props.maxRow*6)), String(props.getRpList(nowIndex+(props.maxRow*6))))
                                        MachineAPI.setReplaceablePartsSettings(Number(prevIndex+(props.maxRow*7)), String(props.getRpList(nowIndex+(props.maxRow*7))))
                                        ...
                                        MachineAPI.setReplaceablePartsSettings(Number(nowIndex+(props.maxRow*4)), String(backUpSize))
                                        MachineAPI.setReplaceablePartsSettings(Number(nowIndex+(props.maxRow*5)), String(backUpAirflow))
                                        MachineAPI.setReplaceablePartsSettings(Number(nowIndex+(props.maxRow*6)), String(backUpPD))
                                        MachineAPI.setReplaceablePartsSettings(Number(nowIndex+(props.maxRow*7)), String(backUpCheck))
                                ...
                                onItemShiftDownClicked: {
                                    if(indexOnList < (props.startIndex + (props.maxRow-1))){
                                        ...
                                        const backUpSize = props.getRpList(nextIndex+(props.maxRow*4))
                                        const backUpAirflow = props.getRpList(nextIndex+(props.maxRow*5))
                                        const backUpPD = props.getRpList(nextIndex+(props.maxRow*6))
                                        const backUpCheck =  props.getRpList(nextIndex+(props.maxRow*7))
                                        ...
                                        MachineAPI.setReplaceablePartsSettings(Number(nextIndex+(props.maxRow*6)), String(props.getRpList(nowIndex+(props.maxRow*6))))
                                        MachineAPI.setReplaceablePartsSettings(Number(nextIndex+(props.maxRow*7)), String(props.getRpList(nowIndex+(props.maxRow*7))))
                                ...
                                onItemClearAllClicked: {
                                    ...
                                    MachineAPI.setReplaceablePartsSettings(Number(indexOnList+(props.maxRow*6)), "")
                                    MachineAPI.setReplaceablePartsSettings(Number(indexOnList+(props.maxRow*7)), "")
                                }//
                            }//
                        }//
                    }//
    ...
    ReplaceablePartsDatabaseApp{
        ...
        Component.onCompleted: {
            //            props.rpDatabase = database[0]['filter']
    ...
    QtObject{
        ...
        readonly property int startIndex: MachineAPI.RPList_Filter1Name
        ...
        function updateRpDatabase(){
            if(MachineData.rpExtDatabaseEnable){
                ...
                props.rpDatabase = dataRp['filter']
            }
            else
                props.rpDatabase = rpApp.database[0]['filter']
        }
        ...
        function setItemCode(saveToIndex, value, force){
            ...
            for(let i=0; i<rpDatabase.length; i++){
                if(rpDatabase[i]['id'] === value){
                    ...
                    size = rpDatabase[i]['size']
                    airflow = rpDatabase[i]['af']
                    pd = rpDatabase[i]['pd']
                    break
                }
            }//
            ...
            /// Set Size if the Item Code Available on Database
            if(itemCodeAvailableOnDatabase){
                MachineAPI.setReplaceablePartsSettings((saveToIndex+(maxRow*3)), size)
                MachineAPI.setReplaceablePartsSettings((saveToIndex+(maxRow*4)), airflow)
                MachineAPI.setReplaceablePartsSettings((saveToIndex+(maxRow*5)), pd)
            }//
        }//
    }//
    Component.onCompleted: {
