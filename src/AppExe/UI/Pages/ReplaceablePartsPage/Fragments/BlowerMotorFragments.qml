...
Item {
                        ...
                        Repeater{
                            ...
                            CusComPage.RowItemApp {
                                ...
                                onLoaded: {
                                    modelColumn = Qt.binding(function(){
                                        let model = [
                                                ...
                                                Number(props.getRpList(indexOnList+(props.maxRow*6))) ? true : false
                                            ]//
                                        return model
                                    })//
                                }//
                                ...
                                onCheckedChanged: {
                                    ...
                                    if((props.getRpList(indexOnList+(props.maxRow*6)) === strVal )
                                            || (strVal === "0" && props.getRpList(indexOnList+(props.maxRow*6)) === "")) return
                                    MachineAPI.setReplaceablePartsSettings(indexOnList+(props.maxRow*6), (value ? "1" : "0"))
                                }
                                onIncompleteData: {
                                ...
                                onItemShiftUpClicked: {
                                    if(indexOnList > props.startIndex){
                                        ...
                                        const backUpSN2 = props.getRpList(prevIndex+(props.maxRow*4))
                                        const backUpSW = props.getRpList(prevIndex+(props.maxRow*5))
                                        const backUpCheck =  props.getRpList(prevIndex+(props.maxRow*6))
                                        ...
                                        MachineAPI.setReplaceablePartsSettings(Number(nowIndex+(props.maxRow*4)), String(backUpSN2))
                                        MachineAPI.setReplaceablePartsSettings(Number(nowIndex+(props.maxRow*5)), String(backUpSW))
                                        MachineAPI.setReplaceablePartsSettings(Number(nowIndex+(props.maxRow*6)), String(backUpCheck))
                                ...
                                onItemShiftDownClicked: {
                                    if(indexOnList < (props.startIndex + (props.maxRow-1))){
                                        ...
                                        const backUpSN2 = props.getRpList(nextIndex+(props.maxRow*4))
                                        const backUpSW = props.getRpList(nextIndex+(props.maxRow*5))
                                        const backUpCheck =  props.getRpList(nextIndex+(props.maxRow*6))
    ...
    QtObject{
        ...
        readonly property int startIndex: MachineAPI.RPList_BMotor1Name
        ...
        function updateRpDatabase(){
            if(MachineData.rpExtDatabaseEnable){
                ...
                props.rpDatabase = dataRp['bMotor']
            }
            else
                props.rpDatabase = rpApp.database[0]['bMotor']
        }//
        ...
        function setItemCode(saveToIndex, value, force){
            ...
            for(let i=0; i<rpDatabase.length; i++){
                if(rpDatabase[i]['id'] === value){
                    ...
                    // Save also the Motor Item Code and quantity if that is 5 series of Blower
                    console.debug((saveToIndex+1), MachineAPI.RPList_BMotor5Code)
                    if(rpDatabase[i]['idm'] !== undefined
                            && (saveToIndex+1) < MachineAPI.RPList_BMotor5Code){ // Not at the last row
                        MachineAPI.setReplaceablePartsSettings(saveToIndex+1, rpDatabase[i]['idm'])
                        MachineAPI.setReplaceablePartsSettings((saveToIndex+maxRow+1), "1")
                    }else{
                        console.debug("This is not Blower Item code!")
                    }
                    break
                }
            }//
