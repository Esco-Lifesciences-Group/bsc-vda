...
ViewApp {
    id: viewApp
    title: "Replaceable Components Record - View"
        ...
        ColumnLayout {
            ...
            Item {
                ...
                HeaderApp {
                    id: headerApp
                    anchors.fill: parent
                    title: qsTr("Replaceable Components Record - View")
                }
            }
            ...
            Item {
                ...
                Loader {
                    ...
                    Component.onCompleted: setSource("Fragments/ProfileFragment.qml", {viewOnly: true})
                }//
                        ...
                        ButtonBarApp {
                            ...
                            onClicked: {
                                ...
                                if(index >= 0){
                                    ...
                                    fragmentLoader.setSource(modelData[index]["link"], {viewOnly: true})
                            ...
                            ListView {
                                ...
                                delegate: Rectangle {
                                    ...
                                    MouseArea {
                                        anchors.fill: parent
                                        onClicked: {
                                            fragmentLoader.setSource(modelData["link"], {viewOnly: true})
                                            pageIndexListView.currentIndex = index
        ...
        QtObject{
            ...
            function addPart(group, itemCode, name, version, serialNumber){
                ...
                if((group === rpApp.group[0])            //sbcSet //QR-Format Plain Text: "Item code#Description#Version#SN"
                        || (group === rpApp.group[2])    //uvLed //QR-Format Plain Text: "Item code#Description#Version#SN"
                        ...
                        || (group === rpApp.group[8]))   //custom //QR-Format Plain Text: "Item code#Description#Version#SN"
                {
                    for(i=0; i<maxRow; i++){
                        ...
                        if(nameEmpty || addQty){
                            ...
                            qty = 1;
                            if(addQty){
                                qty = Number(props.getRpList(Number(((startEnum+i) + (2*maxRow)))))
                                qty++
                            }//
                            ...
                            if(nameEmpty){
                                ...
                                if(String(itemCode) == String("1081370")){
                                    ...
                                    /// Add part Heatsink for Esco SBC Wi-Fi
                                    itemCode = "1081372"
                                    for(let j=0; j<props.rpDatabase['sbcSet'].length; j++){
                                        if(props.rpDatabase['sbcSet'][j]['id'] === itemCode){
                                            name = props.rpDatabase['sbcSet'][j]['desc']
                                            break
                                        }//
                                    }//
                                    savedItemCode = props.getRpList(Number((startEnum+i+1) + (1*maxRow)))
                                    nameEmpty = String(props.getRpList(Number(startEnum+i+1))) == String("")
                                    addQty = String(savedItemCode) == String(itemCode)

                                    //console.debug("savedItemCode", savedItemCode, "new", itemCode, addQty)
                                    if(nameEmpty || addQty){
                                        if(nameEmpty){
                                            MachineAPI.setReplaceablePartsSettings(startEnum+i+1, name)
                                            MachineAPI.setReplaceablePartsSettings(((startEnum+i+1) + (1*maxRow)), itemCode)
                                        }//
                                        qty = 1;
                                        if(addQty){
                                            qty = Number(props.getRpList(Number(((startEnum+i+1) + (2*maxRow)))))
                                            qty++
                                        }//
                                        MachineAPI.setReplaceablePartsSettings(((startEnum+i+1) + (2*maxRow)), String(qty))
                                        if(nameEmpty){
                                            //No Serial Number for Heatsink
                                            MachineAPI.setReplaceablePartsSettings(((startEnum+i+1) + (3*maxRow)), String(""))
                                        }//
                                    }//
                                }//
                            }//
