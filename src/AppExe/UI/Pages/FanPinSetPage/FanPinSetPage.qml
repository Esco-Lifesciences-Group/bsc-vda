...
ViewApp {
    ...
    content.sourceComponent: ContentItemApp {
                ...
                RowLayout {
                    ...
                    Item {
                        ...
                        TextField {
                            ...
                            maximumLength: props.pinDigitTotal
                            ...
                            onTextChanged: {
                                if (text.length == props.pinDigitTotal) {
                                    if (props.newpinConfirm){
                                        if (props.newpinFirst == pinTextField.text){
                                            ...
                                            let isNew = (MachineData.fanPIN == Qt.md5(props.pinReset))
                                            let isChanged = (MachineData.fanPIN != Qt.md5(props.userBlowerPIN))

                                            /// tell to backend
                                            MachineAPI.setFanPIN(props.userBlowerPIN);

                                            if(props.userBlowerPIN !== props.pinReset){
                                                viewApp.showBusyPage( qsTr("Setting up..."),
                                                                     function onTriggered(cycle){
                                                                         if(cycle >= MachineAPI.BUSY_CYCLE_1
                                                                                 || MachineData.fanPIN === props.userBlowerPIN){
                                                                             viewApp.dialogObject.close()
                                                                             if(isNew)
                                                                                 MachineAPI.insertEventLog(qsTr("User: Fan PIN has been created."))
                                                                             else{
                                                                                 if(isChanged)
                                                                                     MachineAPI.insertEventLog(qsTr("User: Fan PIN has been changed."))
                                                                                 else
                                                                                     MachineAPI.insertEventLog(qsTr("User: Fan PIN has been set."))
                                                                             }
                                                                             viewApp.showDialogMessage(qsTr("Notification"),
                                                                                                       (isNew ? qsTr("PIN has been created!")
                                                                                                              : (isChanged ? qsTr("PIN has been changed!")
                                                                                                                           : qsTr("PIN has been set!"))),
                                                                                                       viewApp.dialogInfo)
                                                ...
                                                viewApp.showBusyPage( qsTr("Setting up..."),
                                                                     function onTriggered(cycle){
                                                                         if(cycle >= MachineAPI.BUSY_CYCLE_1
                                                                                 || MachineData.fanPIN === props.userBlowerPIN){
                                                                             viewApp.dialogObject.close()
                                                                             MachineAPI.insertEventLog(qsTr("User: Fan PIN has been reset."))
                                                                             viewApp.showDialogMessage(qsTr("Notification"),
        ...
        QtObject {
            ...
            readonly property int pinDigitTotal: 4
            readonly property string pinReset: "0000"
        }
