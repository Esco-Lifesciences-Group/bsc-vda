...
QtObject {
    ...
    function strfMinToHumanReadableShort(minute){
        ...
        if(Math.floor(minute / 60)){
            ...
            hourStr = hour + " " + (hour > 1 ? qsTr("hours") : qsTr("hour"))
        }

        let minuteStr = ""
        if (Math.floor(minute % 60)){
            let min = Math.floor(minute % 60)
            minuteStr = min + " " + qsTr("min")
        }

        let result = [hourStr, minuteStr].filter(Boolean).join(" ")
        if (result.length === 0) result = qsTr("0 min")
    ...
    function strfSecsToHumanReadableShort(seconds){
        ...
        if (result.length === 0) result = qsTr("0 sec")
    ...
    function strfSecsToHumanReadable(seconds){
        ...
        if (seconds >= 60){
            let min = Math.floor((seconds / 60) % 60)
            if(min > 1) {
                minuteStr = min + " " + qsTr("minutes")
            }
            else if (min === 1) {
                minuteStr = min + " " + qsTr("minute")
            }
        }
        ...
        if(sec) {
            if(sec > 1) {
                secStr = sec + " "+ qsTr("seconds")
            }
            else if (sec === 1) {
                secStr = sec + " " + qsTr("second")
            }
        }

        let result = [hourStr, minuteStr, secStr].filter(Boolean).join(" ")
        if (result.length === 0) result = qsTr("0 second")
