...
CREATE TABLE IF NOT EXISTS datalog_V3 \
    (date TEXT, time TEXT, temp TXT, ifa TXT, dfa TXT, adcIfa INT, pressure TXT, fanRPM INT, fanDcy INT, username TXT, userfullname TXT)"
...
#define DB_QUERY_SELECT                     "SELECT rowid,* FROM datalog_V3"
...
bool DataLogSqlGet::querySelect(QVariantList *data, const QString &dbQueryConfig)
{
    ...
    if(query.exec()){
        ...
        while(query.next()){
            ...
            dataItem.push_back(query.value(TableHeaderEnum::TH_PRESSURE));
            dataItem.push_back(query.value(TableHeaderEnum::TH_FAN_RPM));
            dataItem.push_back(query.value(TableHeaderEnum::TH_FAN_DCY));
