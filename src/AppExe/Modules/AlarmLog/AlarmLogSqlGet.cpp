...
#define DB_QUERY_SELECT                     "SELECT rowid,* FROM alarmlog_V1"

AlarmLogSqlGet::AlarmLogSqlGet(QObject *parent) : QObject(parent)
{

}

AlarmLogSqlGet::~AlarmLogSqlGet()
{
    QSqlDatabase::removeDatabase(m_connectionName);
}

bool AlarmLogSqlGet::init(const QString &uniqConnectionName, const QString &fileName)
{
...
//bool AlarmLogSqlGet::queryInsert(const QVariantMap data)
//{
//    qDebug () << __FUNCTION__ << thread();
...
bool AlarmLogSqlGet::querySelect(QVariantList *data, const QString &dbQueryConfig)
{
    qDebug () << __FUNCTION__ << thread();
    ...
    if(query.exec()){
        ...
        while(query.next()){
            ...
            dataItem.push_back(query.value(TableHeaderEnum::TH_PRIORITY));
            ...
            dataItem.push_back(query.value(TableHeaderEnum::TH_ACKNOTE));
...
bool AlarmLogSqlGet::queryDelete(const QString &dbQueryConfig)
{
    qDebug () << __FUNCTION__ << thread();
...
bool AlarmLogSqlGet::queryCount(int *count)
{
    bool success = false;
...
QString AlarmLogSqlGet::lastQueryErrorStr() const
{
