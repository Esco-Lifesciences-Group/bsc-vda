...
void BookingScheduleQmlApp::updateUsername(const QString &username, const QString &newUsername)
{
    qDebug() << __FUNCTION__ << username << newUsername;
    QMetaObject::invokeMethod(m_pSql.data(),
                              [&, username, newUsername](){
                                  ...
                                  if(username != newUsername)
                                      done &= m_pSql->queryUpdateUsername(newUsername, username);
                                  ...
                                  emit bookingScheduleUsernameUpdated(done, username, newUsername);
                              });
}//
                                       ...
                                       const QString &cabinetModel)
{
    QMetaObject::invokeMethod(m_pSql.data(), [
        ...
        short dayOfWeek = qTargetDate.dayOfWeek();
        ...
        //            item.insert("username", QString("2021-04-11 08:15:00"));
        ...
        for (int j=0; j<totalPagesNeedToExport; j++){
            ...
            for (int i=pageSection; i<pageSectionLimit; i++) {
                ...
                rect.setRect(colBegin4, textMarginTop, textWidthRectColumn4, textHeightRect);
                pPdfPainter->drawRect(rect);
                QString username = dataHolderMap.value("username").toString();
                textContent = dataHolderMap.value("bookBy").toString();// + " (" + username + ")";
                if(textContent != "")
                    textContent += " (booked by: " + username + ")";
                pPdfPainter->drawText(rect, textContent, textOption);
