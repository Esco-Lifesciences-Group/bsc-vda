    ...
    QMetaObject::invokeMethod(m_pSql.data(), [&, pageNumber](){
        ...
        if(done) {
            ...
            for(int i=0; i < logBuffer.length(); i++){
                ...
                for(short j=TableHeaderEnum::TH_ROWID; j<TableHeaderEnum::TH_Total; j++)
                    item.insert(m_tableHeaderString[j], itemTemp.at(j));
...
void ReplaceableCompRecordQmlApp::deleteWhereOlderThanDays(int days)
{
    ...
    QMetaObject::invokeMethod(m_pSql.data(), [&, days](){
        qDebug() << __func__ << thread();
...
void ReplaceableCompRecordQmlApp::queryDeleteAtRowId(int descRowId)
{
    qDebug() << __func__ << descRowId << thread();

    QMetaObject::invokeMethod(m_pSql.data(), [&, descRowId](){
        qDebug() << __func__ << thread();

        bool done = m_pSql->queryDeleteAtRowId(descRowId);
        setLastQueryError(!done);
                                             ...
                                             int endPageNumber)
{
    ...
    QMetaObject::invokeMethod(m_pSql.data(), [
        ...
        QMarginsF margins = QMarginsF(10, 5, 5, 15);
...
void ReplaceableCompRecordQmlApp::filterEmptyStringString(QVariantList value)
{
    QVariantList filterData;
    QVariantList data = value;
    QString str[TH_Total];
    short k = 0;

    const short allQtyParameter[TH_Total]={
        TH_SBCSet1Qty,
        TH_SBCSet2Qty,
        TH_SBCSet3Qty,
        TH_SBCSet4Qty,
    ...
    for(short i=TH_ROWID; i<TH_Total; i++){
        if(i == allQtyParameter[k]){
            str[i] = data.at(i).toInt() > 0 ? data.at(i).toString() : "";
            k++;
        }
        else{
            str[i] = data.at(i).toString();
        }

        // Add "NA" to all column except Name and Check only if its name column is filled
        if(str[i] == ""){
            if(!isColumnName(i)){
                /// SBC SET -> (Item Code ~ Software)
                short indexNameInRow = 0;
                if(i >= TH_SBCSet1Code && i <= TH_SBCSet15SW){
                    const short totalRow = 15;
                    if(i <= TH_SBCSet15Code)
                        indexNameInRow = i - totalRow;
                    else if(i <= TH_SBCSet15Qty)
                        indexNameInRow = i - (2*totalRow);
                    else if(i <= TH_SBCSet15SN)
                        indexNameInRow = i - (3*totalRow);
                    else
                        indexNameInRow = i - (4*totalRow);
                }
                else if(i >= TH_Sensor1Code && i <= TH_Sensor5Const){
                    const short totalRow = 5;
                    if(i <= TH_Sensor5Code)
                        indexNameInRow = i - totalRow;
                    else if(i <= TH_Sensor5Qty)
                        indexNameInRow = i - (2*totalRow);
                    else if(i <= TH_Sensor5SN)
                        indexNameInRow = i - (3*totalRow);
                    else
                        indexNameInRow = i - (4*totalRow);
                }
                else if(i >= TH_UVLED1Code && i <= TH_UVLED6SN){
                    const short totalRow = 6;
                    if(i <= TH_UVLED6Code)
                        indexNameInRow = i - totalRow;
                    else if(i <= TH_UVLED6Qty)
                        indexNameInRow = i - (2*totalRow);
                    else
                        indexNameInRow = i - (3*totalRow);
                }
                else if(i >= TH_PSU1Code && i <= TH_PSU5SN){
                    const short totalRow = 5;
                    if(i <= TH_PSU5Code)
                        indexNameInRow = i - totalRow;
                    else if(i <= TH_PSU5Qty)
                        indexNameInRow = i - (2*totalRow);
                    else
                        indexNameInRow = i - (3*totalRow);
                }
                else if(i >= TH_MCBEMI1Code && i <= TH_MCBEMI5SN){
                    const short totalRow = 5;
                    if(i <= TH_MCBEMI5Code)
                        indexNameInRow = i - totalRow;
                    else if(i <= TH_MCBEMI5Qty)
                        indexNameInRow = i - (2*totalRow);
                    else
                        indexNameInRow = i - (3*totalRow);
                }
                else if(i >= TH_ContactSw1Code && i <= TH_ContactSw5SN){
                    const short totalRow = 5;
                    if(i <= TH_ContactSw5Code)
                        indexNameInRow = i - totalRow;
                    else if(i <= TH_ContactSw5Qty)
                        indexNameInRow = i - (2*totalRow);
                    else
                        indexNameInRow = i - (3*totalRow);
                }
                else if(i >= TH_BMotor1Code && i <= TH_BMotor5SW){
                    const short totalRow = 5;
                    if(i <= TH_BMotor5Code)
                        indexNameInRow = i - totalRow;
                    else if(i <= TH_BMotor5Qty)
                        indexNameInRow = i - (2*totalRow);
                    else if(i <= TH_BMotor5SNMotor)
                        indexNameInRow = i - (3*totalRow);
                    else if(i <= TH_BMotor5SNBlower)
                        indexNameInRow = i - (4*totalRow);
                    else
                        indexNameInRow = i - (5*totalRow);
                }
                else if(i >= TH_CapInd1Code && i <= TH_CapInd5SN){
                    const short totalRow = 5;
                    if(i <= TH_CapInd5Code)
                        indexNameInRow = i - totalRow;
                    else if(i <= TH_CapInd5Qty)
                        indexNameInRow = i - (2*totalRow);
                    else
                        indexNameInRow = i - (3*totalRow);
                }
                else if(i >= TH_Custom1Code && i <= TH_Custom8SN){
                    const short totalRow = 8;
                    if(i <= TH_Custom8Code)
                        indexNameInRow = i - totalRow;
                    else if(i <= TH_Custom8Qty)
                        indexNameInRow = i - (2*totalRow);
                    else
                        indexNameInRow = i - (3*totalRow);
                }
                else if(i >= TH_Filter1Code && i <= TH_Filter5PD){
                    const short totalRow = 5;
                    if(i <= TH_Filter5Code)
                        indexNameInRow = i - totalRow;
                    else if(i <= TH_Filter5Qty)
                        indexNameInRow = i - (2*totalRow);
                    else if(i <= TH_Filter5SN)
                        indexNameInRow = i - (3*totalRow);
                    else if(i <= TH_Filter5Size)
                        indexNameInRow = i - (4*totalRow);
                    else if(i <= TH_Filter5Airflow)
                        indexNameInRow = i - (5*totalRow);
                    else
                        indexNameInRow = i - (6*totalRow);
                }//

                ////
                if(str[indexNameInRow] != ""){
                    str[i] = "NA";
                }//
            }//
        }//

        filterData.append(str[i]);
    }//

    emit filterEmptyStringFinished(filterData);
}
    ...
    if (html.exists()) {
        ...
        for(i=0; i<listTemp.length(); i++){
            ...
            if(i == listTemp.length()-1){
                ...
                QString contentStr = listTemp.at(i);
                QString splitStr = "<div class='pageBreak'></div>";
                QString content1 = contentStr.split(splitStr)[0];
                QString content2 = contentStr.split(splitStr)[1];
                contentStr = content1 + "\n" + splitStr + "\n" + content2;
                                             ...
                                             int row, int page, int maxPage)
{
    ...
    const short allCheckParameter[TH_Total]={
        TH_SBCSet1Check,
        TH_SBCSet2Check,
        TH_SBCSet3Check,
        TH_SBCSet4Check,
    ...
    for(short i=TH_UnitModel; i<totalParameter; i++){
        ...
        if(replacementText[i-1] == ""){
            if(!isColumnName(i)){
                ...
                ////
                if(replacementText[indexNameInRow-1] != ""){
                    replacementText[i-1] = "NA";
                }//
            }//
        }//
...
bool ReplaceableCompRecordQmlApp::isColumnName(int index)
{
    return ((index >= TH_SBCSet1Name && index <= TH_SBCSet15Name)
            || (index >= TH_Sensor1Name && index <= TH_Sensor5Name)
            || (index >= TH_UVLED1Name && index <= TH_UVLED6Name)
            || (index >= TH_PSU1Name && index <= TH_PSU5Name)
            || (index >= TH_MCBEMI1Name && index <= TH_MCBEMI5Name)
            || (index >= TH_ContactSw1Name && index <= TH_ContactSw5Name)
            || (index >= TH_BMotor1Name && index <= TH_BMotor5Name)
            || (index >= TH_CapInd1Name && index <= TH_CapInd5Name)
            || (index >= TH_Custom1Name && index <= TH_Custom8Name)
            || (index >= TH_Filter1Name && index <= TH_Filter5Name));
}//
