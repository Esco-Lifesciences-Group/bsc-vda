#include "ReplaceableCompRecordSqlGet.h"
#include <QProcess>
...
CREATE TABLE IF NOT EXISTS replaceablecomprecord_V2 \
(unitModel TEXT,\
...
//#define DB_QUERY_ADD "INSERT INTO replaceablecomprecord_V2 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"

#define DB_QUERY_DELETE                     "DELETE FROM replaceablecomprecord_V2"
#define DB_QUERY_DELETE_AT_ROWID            "DELETE FROM replaceablecomprecord_V2 WHERE ROWID = (SELECT ROWID FROM replaceablecomprecord_V2 ORDER BY ROWID DESC LIMIT 1 OFFSET %d)"
#define DB_QUERY_COUNT_ROWS                 "SELECT COUNT(*) FROM replaceablecomprecord_V2"

#define DB_QUERY_SELECT                     "SELECT rowid,* FROM replaceablecomprecord_V2"

ReplaceableCompRecordSqlGet::ReplaceableCompRecordSqlGet(QObject *parent) : QObject(parent)
{

}

ReplaceableCompRecordSqlGet::~ReplaceableCompRecordSqlGet()
{
    qDebug() << "~ReplaceableCompRecordSqlGet";
    QSqlDatabase::removeDatabase(m_connectionName);
}

bool ReplaceableCompRecordSqlGet::init(const QString &uniqConnectionName, const QString &fileName)
{
...
//bool ReplaceableCompRecordSqlGet::queryInsert(const QVariantMap data)
//{
//    qDebug () << __FUNCTION__ << thread();
...
bool ReplaceableCompRecordSqlGet::querySelect(QVariantList *data, const QString &dbQueryConfig)
{
    qDebug () << __FUNCTION__ << thread();
    ...
    if(query.exec()){
        ...
        while(query.next()){
            ...
            for(short i=TableHeaderEnum::TH_ROWID; i<TableHeaderEnum::TH_Total; i++)
                dataItem.push_back(query.value(i));
...
bool ReplaceableCompRecordSqlGet::querySelect(QStringList *data, const QString &dbQueryConfig)
{
    qDebug () << __FUNCTION__ << thread();
...
bool ReplaceableCompRecordSqlGet::queryDelete(const QString &dbQueryConfig)
{
    qDebug () << __FUNCTION__ << thread();
...
bool ReplaceableCompRecordSqlGet::queryDeleteAtRowId(int descRowId)
{
    qDebug () << __FUNCTION__ << thread();
    ...
    QString statement = QString().asprintf(DB_QUERY_DELETE_AT_ROWID, descRowId);
...
bool ReplaceableCompRecordSqlGet::queryCount(int *count)
{
    bool success = false;
...
QString ReplaceableCompRecordSqlGet::lastQueryErrorStr() const
{
